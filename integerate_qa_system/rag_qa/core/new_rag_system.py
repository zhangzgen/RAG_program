# 在rag_system的基础上对生成答案的函数进行修改 -> 考虑历史对话, 以及输出采用stream流式输出
...
class RAGSystem:
        ...
        try:
            ...
            for sub_q in subqueries:
                ...
                docs = self.vector_store.hybrid_search_with_rerank(
                    sub_q, k=conf.RETRIEVAL_K // 2, source_filter=source_filter  # 使用 K
                )
    ...
    def generate_answer(self, query, source_filter=None, history=None):
        ...
        if history is not None and not isinstance(history, list):
            logger.warning(f'无效的历史对话格式: {type(history)}, 正确类型: list')
        elif history:  # 取最近5轮历史对话
            history = history[-5:]
        # 拼接历史对话
        history_context = ''
        if history:
            history_context = "\n".join([f"Q:{h['query']}\nA:{h['answer']}" for h in history])
            logger.info(f'使用历史对话: {history_context[:50]}.......')
        #   判断查询类型
        ...
        prompt_input = self.rag_prompt.format(
            context=context, question=query, history=history_context, phone=conf.CUSTOMER_SERVICE_PHONE
        )
        # logger.debug(f"最终生成的 Prompt:\n{prompt_input}") # Debug 日志
        # 限制prompt长度, 避免超过模型最大输入长度
        prompt_input = prompt_input[:self.max_length]
        logger.info(f'检索耗时: {time.time() - start_time:.2f}s')
        try:
